using HW_09_03.Booking.Com.Models;
using System;
using System.Collections.Generic;
using System.Globalization;

namespace HW_09_03.Booking.Com
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            void SearchUser(string login, string pass)
            {
                ...
                {
                    if (login.Equals(accounts[i].Login) && pass.Equals(accounts[i].Password))
                    {
                        Console.WriteLine("Hello, " + accounts[i].Login + "!");
                        break;
                    }
                    else if (login.Equals(accounts[i].Login) && !pass.Equals(accounts[i].Password))
                    {
                        Console.WriteLine("Wrong pass!. Please, try again.");
                        while (true)
                        {
                            pass = Console.ReadLine();
                            if (pass.Equals(accounts[i].Password))
                            {
                                break;
                            }
                            else
                    ...
                    else if (!accounts[i].Login.Contains(login))
                    {
